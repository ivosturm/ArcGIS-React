...
export function _createQueryDefinition(layerObj: LayerArrayType, gisObjects: GisObject[]): string {
    ...
    if (gisObjects.length > 0 && gisObjects.length < 4000 && layerObj.mendixLayer) {
        ...
        // console.debug("queryDefinition for layer: " + layerObj.layerID + " : " + queryDefinition);
    } else {
...
export function highlightAndZoom(
    ...
    load().then(() => {
        ...
        const crimeLayer = new FeatureLayer({
            url: "https://demographics5.arcgis.com/arcgis/rest/services/USA_Crime/MapServer/46"
        });
        const crimeQuery = crimeLayer.createQuery();
        crimeQuery.where = "CRMCYTOTC<88"; // Set by select element
        crimeQuery.spatialRelationship = "intersects"; // Relationship operation to apply
        crimeQuery.geometry = view.extent; // Restricted to visible extent of the map
        crimeQuery.outFields = ["OBJECTID", "CRMCYTOTC", "CRMCYROBB", "NAME"]; // Attributes to return
        crimeQuery.returnGeometry = true;

        crimeLayer
            .queryFeatures(crimeQuery)
            .then(results => {
                console.warn(logNode + results.features.length + " features agreeing with query");
                console.dir(results.features);
                if (results && results.features && results.features.length < 2000) {
                    console.warn(logNode + "less than 2000 features filtered, intersecting..");
                    results.features.forEach(feature => {
                        const intersectingGeometries = geometryEngine.intersect(geometriesReturned, feature.geometry);
                        // insectingGeometries can either be an object or an array
                        if (intersectingGeometries) {
                            if (Array.isArray(intersectingGeometries)) {
                                if (intersectingGeometries.length > 0) {
                                    let countIntersectingGeometries = 0;
                                    intersectingGeometries.forEach(geometry => {
                                        if (geometry) {
                                            countIntersectingGeometries++;
                                        }
                                    });
                                    if (countIntersectingGeometries > 0) {
                                        console.debug(
                                            logNode +
                                                countIntersectingGeometries +
                                                " intersecting with area " +
                    ...
                    console.warn(logNode + "more than 2000 features filtered, NOT intersecting..");
                }
            })
            .catch(error => {
...
/**
 * A simple function to guarantee mendix layers will get a higher index than non-Mendix layers,
 * hence are always plotted on top of non-Mendix layers. Better implementation would be to add an index attribute
 * to both static as well as dynamic layers to get complete freedom of overlays
 */
export const determineIndex = (layerObj: LayerArrayType): number => {
    let index = 0;
                    ...
                    if (view && queryDef) {
                        ...
                        if (layerViewCurrent && layerViewCurrent.layer) {
                            ...
                            if (!props.dsShowAllObjects) {
                                console.debug(
                                    logNode +
                                        ...
                                        layerViewCurrent.layer.id +
                                        " to " +
                                        queryDef.queryDefinitionOriginal
                                );
                                layerViewCurrent.layer.definitionExpression = queryDef.queryDefinitionOriginal;
                                // definitionExpression = queryDef.queryDefinitionOriginal;

                                console.debug(
                                    logNode +
                                        "layerview " +
                                        layerViewCurrent.layer.id +
                                        " resetting definition expression.."
                                );
                            }
                            ...
                            if (
                                ...
                                layerViewCurrent.layer.renderer = renderer;
                                console.debug(
                                    logNode +
                                        "layerview " +
                                        layerViewCurrent.layer.id +
                            ...
                            // });
                            layerViewCurrent.layer.featureReduction = null;
                            const query = layerViewCurrent.layer.createQuery();
                            ...
                            const promiseQuery = new Promise((resolve, reject) => {
                                layerViewCurrent.layer
                                    ...
                                    .then((result: any) => {
                                        ...
                                        whenFalseOnce(layerViewCurrent, "updating", updating => {
                                            console.debug(
                                                logNode + qfResult.layerName + " layer updating " + updating + " once"
                                            );
                                            resolve(qfResult);
                                        });
                                    })
            ...
            Promise.all(promisesLayerQuery)
                .then(results => {
                    ...
                    Promise.all(
                        ...
                        console.debug(logNode + "GoTo done loading");
                        window.setTimeout(() => {
                            // layerViewCurrent.layer.definitionExpression = definitionExpression;
                            console.debug(logNode + "refreshing layer" + layerViewCurrent.layer.title);
                            layerViewCurrent.layer.refresh();
                            /* view.goTo({
                               zoom: view.zoom + 1
                           });
                           view.goTo({
